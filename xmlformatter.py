...
class Formatter:
  ...
  def __init__(self, indent=DEFAULT_INDENT, preserve=[],
               compress=DEFAULT_COMPRESS, indent_char=DEFAULT_INDENT_CHAR,
               encoding_input=DEFAULT_ENCODING_INPUT,
               encoding_output=DEFAULT_ENCODING_OUTPUT, inline=DEFAULT_INLINE,
  ...
  class TokenList:
    ...
    def __init__(self, formatter):
      ...
      for pattern in ['XmlDecl%s', 'AttlistDecl%s', \
                      'StartElement%s', 'EndElement%s', \
    ...
    def token_indent_inline(self, tk):
      ...
      for itk in iter(self.sequence(tk, "EndElement")):
        if itk.level < tk.level and itk.name == "StartElement":
          ...
          return False
        if (
            itk.level == tk.level and tk.name == "EndElement" and itk.name == "StartElement"):
          if itk.content_model == 1:
            return True
          return False
      return True
    ...
    def token_preserve(self, tk):
      ...
      # Unlock preserving for EndElements:
        ...
        if (tk.arg[
          0] in self.formatter.preserve and tk.level == self.preserve_level):
    ...
    def whitespace_append(self, tk, start="StartElement", stop="EndElement",
      ...
      for itk in self.sequence(tk, start):
        if (itk.empty or (
            itk.name == stop and itk.descendant_mixed is False) or (
            itk.name == start and abs(tk - itk) == 1)):
          break
    ...
    def whitespace_delete_leading(self, tk):
      ...
      if (
          self.formatter.correct and tk.leading and not tk.preserve and not tk.cdata_section):
    ...
    def whitespace_delete_trailing(self, tk):
      """Returns True, if no next token or all empty (up to next end element)"""
      if (
          self.formatter.correct and tk.trailing and not tk.preserve and not tk.cdata_section):
        for itk in self.sequence(tk, "StartElement"):
          if itk.end:
            return True
          elif (itk.name in ["StartElement",
                             "StartCdataSection"] or itk.not_empty):
            return False
        return True
      return False
  ...
  class Token(object):
    ...
    def not_empty(self):
      return (
          self.name == "CharacterData" and not self.cdata_section and not re.match(
          r'^[\t\s\n]+$', self.arg[0]))
    ...
    def indent_create(self, times=1):
      ...
      if not self.formatter.compress and self.formatter.indent:
        #print("%s tag have wrong indentation level" % self.arg[0])
        return "\n%s" % (
  ...
  class EndElement(Token):
    ...
    def __unicode__(self):
      ...
      if (not self.formatter.compress or self.list[
        self.pos - 1].name != "StartElement"):
        if self.preserve in [0] and self.indent:

          indent = self.indent_insert()
          if indent != "":
            print("</%s> should occupy a new line" % self.arg[0])
            str += indent
        str += "</%s>" % self.arg[0]
      return str
  ...
  class XmlDecl(Token):
    ...
    def __unicode__(self):
      str = "<?xml%s%s" % (self.attribute('version', self.arg[0]),
                           self.attribute('encoding',
                                          self.formatter.encoding_effective))
      if self.arg[2] > -1:
...
def cli():
  ...
  try:
    opts, args = getopt.getopt(sys.argv[1:], "",
                               ["compress", "disable-correction",
                                "disable-inlineformatting", "encoding=", "help",
                                "infile=", "indent=", "indent-char=",
                                "outfile=", "outencoding=", "preserve="])
  ...
  for key, value in opts:
    if key in ["--indent"]:
      ...
      correct = False
  try:
    formatter = Formatter(indent=indent, preserve=preserve, compress=compress,
                          encoding_input=encoding, encoding_output=outencoding,
                          indent_char=indent_char, inline=inline,
                          correct=correct)
    if infile:
